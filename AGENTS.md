...
Do not log clipboard contents, base64 images, API responses, embeddings, or secrets. Keep endpoint URLs and model names configurable. `smahties` skips `.git`, `.smahties`, `target`, `node_modules`, `.next`, `.turbo`, large files, binary/non-UTF-8 files, and paths matching `.gitignore`; treat `.gitignore` as an indexing exclusion even for tracked files. Indexing can be expensive on large trees.
