...
Keep the two `smahtiepants` agent skills transport-specific. `skills/smahtiepants` must use the CLI and read full pages directly with `smahtiepants docs page`; it should not require `serve`, REST, or MCP. `skills/smahtiepants-mcp` must stay on MCP with `get_page_content` or MCP resources and must not shell out or switch to REST.
