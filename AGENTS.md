...
For `ddserve docs update` and embedding refresh throughput, keep chunk/request sizing bounded and embedding concurrency explicit and conservative. Prefer request-aware batching by input count and bytes, bounded async embedding calls instead of unbounded task fan-out, SQLite WAL plus busy timeout for concurrent readers with serialized writes, and one deferred Annoy rebuild after multi-docset updates rather than rebuilding after every docset. Async OpenAI/httpx clients created by ddserve must be created and closed inside the same running event loop; externally injected async clients remain caller-managed.
